    ...
    @Operation(
        ...
        description = """
            ...
            **하네스 3계층 처리 순서**
            ...
            2. DB에서 tagCode → modelCode 조회 (프론트가 modelCode를 직접 지정할 수 없음)
            3. AI 서버(RAG 기반) SSE 스트리밍 호출 → 클라이언트 연결 끊김 시 서버가 구독 즉시 취소

            **가드레일(가품 판정 / 가격 산정 / 리셀 시세 언급 금지)은 AI 서버가 자체 RAG·프롬프트로 처리합니다.**
            백엔드는 modelCode + 사용자 메시지만 AI 서버에 전달하며, 별도의 시스템 프롬프트를 조립해 보내지 않습니다.
